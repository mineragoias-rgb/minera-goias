...
function yearLabel(){const v=yearValue();el('atlas-year-label').textContent=v==='total'?t('at.accum'):v;
 const ticks=el('atlas-year-ticks');
 if(!ticks.children.length)ticks.innerHTML=YEARS.map((y,i)=>`<button type="button" data-year-index="${i}">${escape(y==='total'?t('at.accumShortLabel'):y)}</button>`).join('');
 [...ticks.children].forEach((b,i)=>{b.classList.toggle('active',YEARS[i]===v);b.setAttribute('aria-pressed',String(YEARS[i]===v));
  b.onclick=()=>{el('atlas-year').value=i;yearLabel();render()}})}
