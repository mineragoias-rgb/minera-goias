...
if(layer==='production'){value=p?p[0]*(gold?1000:1):null;note=sub;details=[[t('at.substance'),sub],[t('at.qtySold'),value==null?t('at.noRecord'):fmt(value,2)+' '+unit],[t('at.cfem'),p?money(p[1]):t('at.noRecord')],[t('at.companies'),p?fmt(p[2]):t('at.noRecord')],[t('at.periodField'),'2025']]}
...
return {name:m.name,cells:[m.name,value==null?t('at.noRecord'):fmt(value,2)+' '+unit,note],value,detail:details,municipality:m}});
...
if(layer==='cfem'&&year!=='total')el('atlas-scale-note').textContent=t('at.cfemYearNote');legend([[t('at.noneZero'),'#edf1f4'],...[scale.min,...scale.cuts].filter(v=>v!=null).map((v,i)=>[t('at.from')+' '+fmt(v,2)+' '+unit,ramp[i]]),...(layer==='coefficient'?Object.entries(special):[])]);
